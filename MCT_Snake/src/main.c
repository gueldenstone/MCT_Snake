...
static volatile _Bool position[8][8]={0},zufall[8][8]={0}, output[8][8]={0};
...
//	TIM6->CR1 |= TIM_CR1_CEN;
	...
	while(1){
		...
		y = ((ADC3->DR)/512);
		if(ADC1->DR>=2030 && ADC1->DR<=2060){
			x=4;
		}
		if(ADC3->DR>=2030 && ADC3->DR<=2060){
			y=4;
		}
		position[x][y] = 1;
		...
		GPIOA->ODR |= (uint32_t)((position[i][j])<<(i+4));		//Spalte entsprechend Wert in matrix setzten
		GPIOC->ODR &= ~((uint32_t)position[i][j]<<j);			//Zeile entsprechend Wert in matrix setzten
		i++;
		if(i>7){j++; i=0;}
		if(j>7){i=0; j=0;}
	}
...
void ADC_Config(void){
	...
	ADC1->CFGR |= 0b00 << ADC_CFGR_RES_Pos;		// Auflösung sind 6 Bit
	...
	ADC3->CFGR |= 0b00 << ADC_CFGR_RES_Pos;		// Auflösung sind 6 Bit
...
void randompoint(void){
	uint8_t randx=rand()%8;
	uint8_t randy=rand()%8;
